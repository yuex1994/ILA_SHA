/// \file Utility function for constructing the model
///  Hongce Zhang (hongcez@princeton.edu)

#include "sha_ila.h"
#include <cmath>

ExprRef SHA::unknown_range(unsigned low, unsigned high) {
...
ExprRef SHA::unknown_choice(const ExprRef& a, const ExprRef& b) {
  return Ite(unknown(1)() == 1, a, b);
}

// input_slice.width == slice_width
// num_slice * slice_width == reg.width
ExprRef SHA::slice_update(const ExprRef& reg, const ExprRef& idx,
...
ExprRef SHA::slice_read(const ExprRef& reg, const ExprRef& idx,
...
FuncRef SHA::unknown(unsigned width) {
